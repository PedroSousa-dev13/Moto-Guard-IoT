...
function getEventCount(eventCounts: Partial<Record<string, number>>, key: string): number {
...
export function categorizeTrip(input: CategorizationInput): CategorizationResult {
  ...
  const trackDayConditions: boolean[] = [
  ...
  const offRoadConditions: boolean[] = [
  ...
  const weekendRideConditions: boolean[] = [
  ...
  const commuteConditions: boolean[] = [
    trip.distanceKm !== null && trip.distanceKm < 30,
  ...
  if (trackDaySatisfied) {
    ...
    if (trackDayConditions[0]) matchedRules.push("maxSpeedKmh>120");
    if (trackDayConditions[1]) matchedRules.push("maxRollDeg>40");
    if (trackDayConditions[2]) matchedRules.push("aggressiveEvents>2");
    return {
      category: "TRACK_DAY",
  ...
  if (offRoadSatisfied) {
    const matchedRules: string[] = [];
    if (offRoadConditions[0]) matchedRules.push("avgSpeedKmh<40");
    if (offRoadConditions[1]) matchedRules.push("maxRollDeg>30");
    if (offRoadConditions[2]) matchedRules.push("highVibration>0");
    return {
      category: "OFF_ROAD",
  ...
  if (weekendRideSatisfied) {
    const matchedRules: string[] = [];
    if (weekendRideConditions[0]) matchedRules.push("distanceKm>=30");
    if (weekendRideConditions[1]) matchedRules.push("avgSpeedKmh_50_100");
    if (weekendRideConditions[2]) matchedRules.push("maxRollDeg<40");
    return {
  ...
  if (commuteSatisfied) {
    const matchedRules: string[] = [];
    if (commuteConditions[0]) matchedRules.push("distanceKm<30");
    if (commuteConditions[1]) matchedRules.push("avgSpeedKmh<50");
    if (commuteConditions[2]) matchedRules.push("speeding=0");
    return {
      category: "COMMUTE",
 ...
 */
export async function categorizeTripById(
  tripId: string,
  userId: string
  ...
  if (!trip) return null;
  if (trip.status !== TripStatus.COMPLETED) return null;

  // Dados insuficientes
  if (trip.distanceKm === null && trip.avgSpeedKmh === null) {
    console.warn(
      `[trip-categorization] Viagem ${tripId} sem dados suficientes (distanceKm e avgSpeedKmh nulos) — ignorada`
    );
    return null;
  }

  // Construir contagem de eventos por tipo
  const eventCounts: Partial<Record<string, number>> = {};
...
/**
 * Reclassifica em lote todas as viagens COMPLETED sem categoria.
 * Retorna { processed, errors } com as contagens.
 */
export async function reclassifyAllUnategorized(): Promise<{
  processed: number;
  errors: number;
}> {
  ...
  for (const trip of trips) {
    try {
      const result = await categorizeTripById(trip.id, trip.userId);
      if (result === null) {
        errors++;
      } else {
        processed++;
      ...
      console.error(
        `[trip-categorization] Erro ao reclassificar viagem ${trip.id}:`,
        err
      );
      errors++;
    }
