...
exports.saveSimulatorGpxData = saveSimulatorGpxData;
...
function computeBounds(waypoints) {
    let minLat = Infinity;
    let maxLat = -Infinity;
    let minLon = Infinity;
    let maxLon = -Infinity;
...
async function saveSimulatorGpxData(req, res) {
    ...
    try {
        ...
        const gpxData = await prisma_service_1.prisma.$transaction(async (tx) => {
