...
class DeviceAssociationService {
    ...
    async getAssociation(deviceId, userId, motoModel) {
        ...
        if (!motorcycle) {
            ...
            if (isSim && userId) {
                const fallback = await prisma_service_1.prisma.motorcycle.findFirst({
    ...
    async registerDevice(deviceId, userId, motorcycleName, profileId) {
        ...
        if (isSim) {
            const anyMotoWithId = await prisma_service_1.prisma.motorcycle.findFirst({
                where: { deviceId, userId }
            });
            if (anyMotoWithId) {
        ...
        if (isSim && !effectiveProfileId) {
            const profile = await prisma_service_1.prisma.motorcycleProfile.findFirst({
